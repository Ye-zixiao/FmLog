...
void getAllocation(int &new_time, int &reuse_time,
  ...
  new_time = g_allocation_time.load();
  reuse_time = g_reuse_time.load();
  backup_push_time = g_backup_push_time.load();
  backup_pop_time = g_backup_pop_time.load();
}
...
LogBuffer::LogBuffer() :
  ...
  // 构造之时只是分配了相应的内存空间
  static_assert(sizeof(Item) == 256, "sizeof Item != 256");
}
...
bool LogBuffer::push(LogLine &&log_line, size_t write_index) {
  ...
  return write_states_[kSize].fetch_add(1, std::memory_order_relaxed) + 1 == kSize;
...
bool RingQueue<SIZE>::push(std::unique_ptr<LogBuffer> &&log_buffer) {
  ...
  fprintf(stderr, "in push, write_index: %zu, read_index: %zu\n", write_index_, read_index_);
...
bool RingQueue<SIZE>::pop(std::unique_ptr<LogBuffer> &log_buffer) {
  SpinLock spin_lock(flag_);
  fprintf(stderr, "in pop, write_index: %zu, read_index: %zu\n", write_index_, read_index_);
...
bool LogBufferQueue::pop(LogLine &log_line) {
  ...
  if (read_buffer->pop(log_line, read_index_)) {
    ...
    if (read_index_ == LogBuffer::kSize) {
      ...
      // 如果读缓冲区中所有的日志行都被读取，则将该读日志行缓冲区从队列中弹出，
      // 但这个缓冲区并不会立即释放掉，而是存放到环形空间缓存中进行保存，以备
      // 压入日志行时复用之
